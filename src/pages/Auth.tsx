...
export default function Auth() {
  ...
  const handleResendOtp = async () => {
    try {
      // Re-send signup OTP by calling signUp again (Supabase resends the confirmation)
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] p-8 md:p-12 w-full max-w-md rounded-xl shadow-2xl">
          ...
          {step === 'auth' && (
            <>
              {isLogin ? (
                ...
                <form onSubmit={handleEmailLogin} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'var(--gold-lo)' }} />
              ...
              <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-white text-black font-bold py-3 hover:bg-gray-200 transition-colors rounded">
              ...
              <div className="mt-6 text-center">
                <button onClick={() => setIsLogin(!isLogin)} className="text-gray-400 text-sm font-medium hover:text-white transition-colors">
                  {isLogin ? "New to WhatToWatchNext? Sign up now." : 'Already have an account? Sign in.'}
                </button>
              </div>
            </>
          )}
